...
if __name__ == '__main__':
    lossm, loss_n, epmin, epstart = 67000, 68000, 0, 0
    # lossn记录，lossm画图
    epend = 800000
    ch, d = 30, 2
    img_size = 200
    ...
    tensor_name = 'VDNN_phase_array/tensor_output_test2.txt'
    ...
    if D_flag == 0:
        ...
        D0 = D0.reshape(ch, 200, 200).cuda()
        D = D0
    if D_flag == 1:
        with open(path + 'VDNN_phase_array/tensor_output_test.txt', 'r') as file:
            for line in file:
                D0.append(list(map(float, line.strip().split())))
        D0 = torch.tensor(D0, dtype=torch.float32)
        D0 = D0.reshape(ch, 200, 200).cuda()
...
#    )
    ...
    # )
    # 定义 AdamW 优化器
    optimizer = optim.AdamW([D], lr=1e-2, weight_decay=1e-3)

    # 余弦退火调度器
    scheduler = CosineAnnealingLR(
        optimizer,
        T_max=100,               # 半周期长度（epoch数）
